...
echo "[TEST] Starting to publish messages. Iterations: ${ITERATIONS}, Workers: ${WORKERS}, Messages per worker: ${MESSAGES_PER_WORKER}"
seq 1 ${ITERATIONS} | parallel -j${WORKERS} "publish_messages ${MESSAGES_PER_WORKER} {}"
echo "[TEST] Messages published to mqtt"
...
for i in `seq 1 ${ITERATIONS}`; do
    ...
    echo "[TEST] Publisher #${i}: ${WORKER_MESSAGES}"
done

if [[ ${NUM_MESSAGES_RECEIVED} -ne ${EXPECTED_NUM_MESSAGES} ]]; then
    echo "[TEST] FAILURE: Expected ${EXPECTED_NUM_MESSAGES} messages received. Was: ${NUM_MESSAGES_RECEIVED}"
    exit 1
fi

METRICS_COUNTER=$(curl -s localhost:8080/metrics | grep "mqtt_client_total_messages" | grep -v "#" | awk '{print $2}')
echo "[TEST] Metrics counter: ${METRICS_COUNTER}"
if [[ ${METRICS_COUNTER} -ne ${EXPECTED_NUM_MESSAGES} ]]; then
    echo "[TEST] FAILURE: Expected metrics counter to be ${EXPECTED_NUM_MESSAGES}. Was: ${METRICS_COUNTER}"
    exit 1
fi