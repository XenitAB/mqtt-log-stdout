...
# Cleanup
echo "[PREPARE] Docker clean up started."
set +e
...
set -e
echo "[PREPARE] Docker clean up finished."


if [[ "${CI}" == "true" ]]; then
    sudo apt-get install mosquitto-clients parallel 1>/dev/null
    echo "[PREPARE] Installed pre-requisites in CI"
fi

if [[ "${CI}" != "true" ]]; then
    docker build -t ${IMG} . 1>/dev/null
    VERSION="dev"
    echo "[PREPARE] Built mqtt-log-stdout outside of CI"
fi

docker network create --driver bridge endtoend 1>/dev/null
docker run --network endtoend -p 1883:1883 -v "$(pwd)"/test/vernemq:/mnt -e "DOCKER_VERNEMQ_VMQ_ACL__ACL_FILE=/mnt/vmq.acl" -e "DOCKER_VERNEMQ_ACCEPT_EULA=yes" -e "DOCKER_VERNEMQ_ALLOW_ANONYMOUS=on" --name vernemq-e2e -d vernemq/vernemq 1>/dev/null
echo "[PREPARE] Started vernemq"

echo "[PREPARE] Validating connection to mqtt"
...
if [ "${VALID_CONNECTION}" = false ]; then
    echo "[PREPARE] FAILURE: Not able to establish connection to MQTT server."
    exit 1
fi

echo "[PREPARE] Connection to mqtt validated"
...
echo "[PREPARE] mqtt-log-stdout started"
